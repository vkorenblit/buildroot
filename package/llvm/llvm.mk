...
# output only $(HOST_LLVM_TARGET_ARCH) if not, and mesa3d won't build as
# it thinks AMDGPU backend is not installed for the target
ifeq ($(BR2_PACKAGE_LLVM_AMDGPU),y)
LLVM_TARGETS_TO_BUILD += AMDGPU
endif

# LLVM target to use for native code generation.
# Set it to "host" for automatic detection on the host.
HOST_LLVM_CONF_OPTS += -DLLVM_TARGET_ARCH="host"
# We are crosscompiling, LLVM_TARGET_ARCH must be set to the target architecture name.
...
# Default is Debug build which requires considerably more disk space and build time.
# Use the Release build for the host. Let the CMake infra handle this option for the
# target.
...
endef
LLVM_POST_INSTALL_TARGETS_HOOKS = LLVM_DELETE_LLVM_TBLGEN_TARGET
